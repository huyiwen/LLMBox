...
class Qianfan(ApiModel):
    ...
    @staticmethod
    def _get_error_type(response: QfResponse) -> Optional[Exception]:
        # https://cloud.baidu.com/doc/WENXINWORKSHOP/s/tlmyncueh
        if "error_code" not in response.body:
            return None
        if response.body["error_code"] in {
            336007,
            336103,
            336122,
    ...
    def _chat_completions(self, **kwargs) -> QfResponse:
        return self.model.do(**kwargs)
