...
from .enum import OPENAI_CHAT_COMPLETIONS_ARGS, OPENAI_COMPLETIONS_ARGS
...
class Openai(ApiModel):
    ...
    def _chat_completions(self, **kwargs):
        results = openai.chat.completions.create(**kwargs)
    ...
    def set_ppl_args(self, **extra_model_args):
        ...
        if self.endpoint == "chat/completions":
            raise ValueError(
                f"{self.name} doesn't support PPL score calculation. Please use get_prob mode by setting `--ranking_type generation` instead"
            )
    ...
    def get_prob(self, batched_inputs: List[Tuple[str, int]]) -> List[List[int]]:
        ...
        *batched_prompts, batched_option_nums = map(list, zip(*batched_inputs))
        ...
        for result, option_num, label in zip(results, batched_option_nums, label_texts):
            result: openai.types.CompletionChoice = result
            if result.logprobs is not None:
                ...
                probs = [-9999.] * (option_num * 2)
                probs[label.index(result.text)] = 20.0
