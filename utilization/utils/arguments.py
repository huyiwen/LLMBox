...
from ..model.enum import (ANTHROPIC_CHAT_COMPLETIONS_ARGS, API_MODELS, DASHSCOPE_CHAT_COMPLETIONS_ARGS,
...
class ModelArguments(ModelBackendMixin):
    ...
    _model_specific_arguments: ClassVar[Dict[str, Set[str]]] = {
        "anthropic": {"anthropic_api_key"} | ANTHROPIC_CHAT_COMPLETIONS_ARGS,
        "dashscope": {"dashscope_api_key"} | DASHSCOPE_CHAT_COMPLETIONS_ARGS,
        "openai": set(),  # openai model is used for gpt-eval metrics, not specific arguments
        "qianfan": {"qianfan_access_key", "qianfan_secret_key"} | QIANFAN_CHAT_COMPLETIONS_ARGS,
    ...
    def __post_init__(self):
        ...
        self.model_name_or_path = self.model_name_or_path.lower()
...
def check_args(model_args: ModelArguments, dataset_args: DatasetArguments, evaluation_args: EvaluationArguments):
    ...
    if model_args.model_name_or_path in API_MODELS and \
            API_MODELS[model_args.model_name_or_path]["endpoint"] == "chat/completions" and \
            dataset_args.ranking_type.startswith("ppl"):
        dataset_args.ranking_type = "prob"
        if dataset_args.passed_in_commandline("ranking_type"):
            logger.warning(
                f"Chat completions endpoint model {model_args.model_name_or_path} doesn't support ppl ranking, automatically set ranking_type to prob."
            )
