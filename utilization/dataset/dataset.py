...
class Dataset(torch.utils.data.Dataset, DatasetUtilMixin):
    ...
    def _init_arguments(self):
        ...
        if self.model.args.api_endpoint == "chat/completions" and self.ranking_type != "generation":
            self.ranking_type = "generation"
            logger.warning(
                f"chat/completions endpoint does not support `logprobs` or `logit_bias`, automatically set ranking_type to `generation`."
            )
